...
export default function CountryGuidesLiteClient({
  countries,
}: {
  countries: Country[];
}) {
  ...
  return (
    <main className="onboarding-bg relative min-h-screen flex flex-col bg-black px-6 py-12 text-white md:px-10">
      ...
      <div className="relative mx-auto w-full max-w-4xl flex-1">
        <h1 className="font-editorial text-5xl text-white">Country Guide Lite</h1>
        <p className="mt-3 text-sm text-white/55">
          Public starter briefings for global mobility and tax planning conversations.
        </p>
        <p className="mt-2 text-xs text-white/45">
          Need live obligations and premium intelligence?{" "}
          <Link
            href="/pricing"
            className="underline decoration-white/45 underline-offset-4 transition-colors hover:text-white/75"
          >
            View Nuvare plans
          </Link>
          .
        ...
        <div className="relative mt-10">
          ...
          <Input
            ...
            aria-label="Search public country guides"
          />
        </div>

        {!normalizedQuery ? (
          <p className="mt-4 text-sm text-white/45">
            Type a country name to open a public guide.
          </p>
        ) : (
