...
function GuideSkeleton() {
  return (
    <div className="mt-8 space-y-3 rounded-2xl border border-white/12 bg-[#0b0b0b]/80 p-6">
...
async function GuideContent({ slug, baseUrl }: { slug: string; baseUrl: string }) {
  const response = await fetch(`${baseUrl}/api/countries/${encodeURIComponent(slug)}/guide`, {
    method: "GET",
    cache: "no-store",
  });
  ...
  if (!response.ok || !payload?.content || !payload.updatedAt) {
    return (
      <p className="mt-8 rounded-xl border border-red-200/20 bg-red-500/10 p-4 text-sm text-red-200">
        {payload?.error ?? "Unable to load country guide."}
      </p>
    );
  }

  const guide = payload as GuidePayload;

  return (
    <>
      <p className="mt-2 text-xs text-white/45">Last updated: {formatLastUpdated(guide.updatedAt)}</p>
    ...
    <main className="onboarding-bg relative min-h-screen overflow-hidden bg-black px-6 py-10 text-white md:px-10">
      ...
      <div className="relative mx-auto w-full max-w-4xl">
        ...
        <Suspense fallback={<GuideSkeleton />}>
          <GuideContent slug={slug} baseUrl={baseUrl} />
        </Suspense>
      </div>
