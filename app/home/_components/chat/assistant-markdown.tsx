    ...
    <ReactMarkdown
      ...
      components={{
        h1: ({ children: c }: ComponentPropsWithoutRef<"h1">) => (
          <h1 className="mb-3 text-lg font-bold text-white tracking-tight">{c}</h1>
        ),
        h2: ({ children: c }: ComponentPropsWithoutRef<"h2">) => (
          <h2 className="mb-2 text-base font-semibold text-white tracking-tight">{c}</h2>
        ),
        ...
        p: ({ children: c }: ComponentPropsWithoutRef<"p">) => (
          <p className="mb-3 text-sm leading-relaxed text-[#e2e2eb] last:mb-0">{c}</p>
        ),
        strong: ({ children: c }: ComponentPropsWithoutRef<"strong">) => (
          <strong className="font-bold text-white">{c}</strong>
        ),
        ul: ({ children: c }: ComponentPropsWithoutRef<"ul">) => (
          <ul className="mb-3 list-disc space-y-1.5 pl-5 text-sm text-[#e2e2eb]">{c}</ul>
        ),
        ol: ({ children: c }: ComponentPropsWithoutRef<"ol">) => (
          <ol className="mb-3 list-decimal space-y-1.5 pl-5 text-sm text-[#e2e2eb]">{c}</ol>
        ),
        li: ({ children: c }: ComponentPropsWithoutRef<"li">) => <li>{c}</li>,
        hr: () => <hr className="my-4 border-white/10" />,
        table: ({ children: c }: ComponentPropsWithoutRef<"table">) => (
          <div className="my-3 overflow-x-auto">
            <table className="w-full border-collapse text-left text-sm text-[#e2e2eb]">{c}</table>
          </div>
        ),
        thead: ({ children: c }: ComponentPropsWithoutRef<"thead">) => (
          <thead className="bg-[#282a30] text-white">{c}</thead>
        ),
        tbody: ({ children: c }: ComponentPropsWithoutRef<"tbody">) => <tbody>{c}</tbody>,
        tr: ({ children: c }: ComponentPropsWithoutRef<"tr">) => (
          <tr className="border-b border-white/8 last:border-b-0">{c}</tr>
        ),
        th: ({ children: c }: ComponentPropsWithoutRef<"th">) => (
          <th className="border border-white/10 px-3 py-2 font-semibold text-white">{c}</th>
        ),
        td: ({ children: c }: ComponentPropsWithoutRef<"td">) => (
          <td className="border border-white/8 px-3 py-2 text-[#e2e2eb]">{c}</td>
        ),
      }}
    >
      {children}
